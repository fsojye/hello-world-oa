...
from models.serializers.world import DeleteWorldRequest, GetWorldResponse, UpdateWorldRequest
...
from services.planet_deleter_service import PlanetDeleterService
...
async def update_world(request: UpdateWorldRequest) -> BaseResponse:
    try:
        ...
        return BaseResponse(message=f"{request.name}, born on {world.date_created}, from {world.date_modified} you shall be known as {request.new_name}")
